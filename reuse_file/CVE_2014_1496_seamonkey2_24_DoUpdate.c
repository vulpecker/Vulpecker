...
int CVE_2014_1496_seamonkey2_24_DoUpdate()
{
  ...
  if (rv) {
    ...
    if (rv) {
      LOG(("CVE_2014_1496_seamonkey2_24_DoUpdate: error extracting manifest file"));
      return rv;
    }
  }
  ...
  if (rb == nullptr) {
    LOG(("CVE_2014_1496_seamonkey2_24_DoUpdate: error opening manifest file: " LOG_S, manifest));
    return READ_ERROR;
  }
  ...
  while((line = mstrtok(kNL, &rb)) != 0) {
    ...
    if (!token) {
      LOG(("CVE_2014_1496_seamonkey2_24_DoUpdate: token not found in manifest"));
      return PARSE_ERROR;
    }
    ...
    if (NS_tstrcmp(token, NS_T("remove")) == 0) { // rm file
      ...
      LOG(("CVE_2014_1496_seamonkey2_24_DoUpdate: unknown token: " LOG_S, token));
      return PARSE_ERROR;
    }
