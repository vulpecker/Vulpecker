...
int CVE_2013_1707_thunderbird17_0_7_NS_main(int argc, NS_tchar **argv)
{
  ...
  if (!GetLongPathNameW(gDestinationPath, applyDirLongPath,
    ...
    LOG(("CVE_2013_1707_thunderbird17_0_7_NS_main: unable to find apply to dir: " LOG_S "\n", gDestinationPath));
    LogFinish();
  ...
  if (argc > callbackIndex) {
    ...
    if (!GetLongPathNameW(targetPath, callbackLongPath,
      ...
      LOG(("CVE_2013_1707_thunderbird17_0_7_NS_main: unable to find callback file: " LOG_S "\n", targetPath));
      LogFinish();
      WriteStatusFile(WRITE_ERROR);
      EXIT_WHEN_ELEVATED(elevatedLockFilePath, updateLockFileHandle, 1);
      if (argc > callbackIndex) {
    ...
    if (!sReplaceRequest) {
      ...
      *d = NS_T('\0');
      ...
      do {
        ...
        LOG(("CVE_2013_1707_thunderbird17_0_7_NS_main: callback app open attempt %d failed. " \
      ...
      if (callbackFile == INVALID_HANDLE_VALUE) {
        LOG(("CVE_2013_1707_thunderbird17_0_7_NS_main: file in use - failed to exclusively open executable " \
             "file: " LOG_S "\n", argv[callbackIndex]));
        LogFinish();
  ...
  if (!sBackgroundUpdate && !sReplaceRequest && _wrmdir(DELETE_DIR)) {
    LOG(("CVE_2013_1707_thunderbird17_0_7_NS_main: unable to remove directory: " LOG_S ", err: %d\n",
         DELETE_DIR, errno));
    ...
    if (MoveFileEx(DELETE_DIR, NULL, MOVEFILE_DELAY_UNTIL_REBOOT)) {
      LOG(("CVE_2013_1707_thunderbird17_0_7_NS_main: directory will be removed on OS reboot: " LOG_S "\n",
           DELETE_DIR));
    } else {
      LOG(("CVE_2013_1707_thunderbird17_0_7_NS_main: failed to schedule OS reboot removal of " \
           "directory: " LOG_S "\n", DELETE_DIR));
    }
  }
#endif /* XP_WIN */
  ...
  if (argc > callbackIndex) {
    ...
    if (gSucceeded) {
      ...
      if (!sUsingService) {
        ...
        if (GetInstallationDir(installDir)) {
          if (!LaunchWinPostProcess(installDir, gSourcePath, false, NULL)) {
            LOG(("CVE_2013_1707_thunderbird17_0_7_NS_main: The post update process could not be launched.\n"));
          }
